...
NAME: \wss{Enter your name here}\\[1ex] Mingzhe Wang

Student ID: \wss{Enter your student number here} \\[2mm] 400316660
...
\item Separation of Concerns
  \begin{itemize}
  \item MG illustrates Separation of Concerns, by writing MG, we decompose the whole system to different module, which provides service and secret.
  \item The test process also illustrates Separation of Concerns, because instead of doing integration testing first, we use unit tests to verify the reliability of each method in a module. By doing this, we can find the flaw in a specific area.
  \item Abstraction is a special case of separation of concerns. Many different models of the same entity can be produced by abstraction
  \end{itemize}
\end{enumerate}
...
\begin{itemize}
\item \textbf{consistency}: The design seems not consistent because of its name conventions. For example. the \verb|empty| should be renamed to \verb|is_empty| to be consistent with \verb|is_member|. In addition, the \verb|to_seq| should be renamed to \verb|get_seq| because it is not a mutator.
\item \textbf{essentiality}: The design is not essential because some services provided a method can be provided by the combination of other method's services. For example, the service of \verb|empty| can be gained by using \verb|size|.
\item \textbf{generality}: The design is also not general. Because we don't know how the module will be used in the future. It is better for us to change the design of set to work for all types instead of just integers.
\item \textbf{minimality}: The design is in general minimal because every methods only provides one service. The only behavior seems not too minimal is that the \verb|union| and \verb|diff| actually return a new \verb|SetOfInt| object.
\end{itemize}
...
\begin{enumerate}[a.]
\item Change the header to \textbf{Generic T Abstract Data Type}.\\ 
Change the Template Module name to \textbf{Set(T)}.\\
Change the Exported Types name to \textbf{Set(T) = ?}.\\
Add \textbf{Generic} before  \textbf{Template Module}.\\
Replace all the type name $\mathbb{Z}$ in the sytax and semantic field to \textbf{T}.
\item Nothing. The implementation in Python is already generic by dynamic typing. (duck typing)
\item The \verb|equal()| relational operator should be defined.
\item (BONUS) Define an  interface that specifies the \verb|equal()| relational operator of Type T, that is \verb|Set(T with Equality(T))| in the header, where \verb|Equality(T)| is the generic interface.
\end{enumerate}
